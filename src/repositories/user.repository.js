...
export const addUser = async (data) => {
	...
	try {
		...
		const [result] = await pool.query(
			`INSERT INTO users (email, name, gender, username, birth, address, detail_address, phone_number) VALUES (?, ?, ?, ?, ?, ?, ?, ?);`,
			[
				data.email,
				data.name,
				data.gender,
...
export const setPreference = async (userId, foodCategoryId) => {
	...
	try {
		await pool.query(
			`INSERT INTO user_preference (food_category_id, user_id) VALUES (?, ?);`,
